...
import '../../../home/ui/controllers/featured_luxury_cars/featured_luxury_cars_bloc_bloc.dart';

class FeaturedCarList extends StatelessWidget {
  const FeaturedCarList({super.key});

  @override
  Widget build(BuildContext context) {
    print("Featured car list Build Running");
    return BlocBuilder<FeaturedLuxuryCarsBlocBloc, FeaturedLuxuryCarsBlocState>(
      ...
      if (state is FeaturedLuxuryCarsBlocInitial) {
        ...
        return SizedBox(
          ...
          child: BlocListener<AddToWishlistCubit, AddToWishlistState>(
            listener: (context, state) {
              if (state is ProductAddedToWishlist) {
                print(state.toString());
                ScaffoldMessenger.of(context)
                    .showSnackBar(SnackBar(content: Text(state.message)));

                // below is code to fetch wishlist //
                context.read<WishListControllerBloc>().add(GetWishListEvent(
                    slug: 'cars',
                    userId: context.read<ProfileBloc>().profileEntity.sId));
              } else if (state is FailedToAddProductWishlist) {
